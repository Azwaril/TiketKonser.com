...
<div class="py-6 bg-gray-900 min-h-screen px-4 text-white rounded-lg">
    ...
    <form action="{{ route('admin.tickets.store') }}" method="POST" class="bg-gray-800 p-6 rounded-lg shadow space-y-4">
        @csrf

        <div>
            <label for="event_id" class="block mb-1">Pilih Event</label>
        ...
        <div>
            ...
            <input type="number" name="price" id="price" required min="0" 
                   class="w-full bg-gray-700 text-white rounded px-3 py-2" value="{{ old('price') }}">
            @error('price')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="stock" id="stock" required min="0"
                   class="w-full bg-gray-700 text-white rounded px-3 py-2" value="{{ old('stock') }}">
            @error('stock')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
